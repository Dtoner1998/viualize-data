...
class Visual():
    ...
    def compYearlyCaseNumbersTrendLines(self, df, disease, begin, end):
        ...
        df = df.groupby(['name','date1']).first().reset_index()
        dfTotalCases = df.groupby(['name','date1']).sum().reset_index()
        dfMean = dfTotalCases.groupby(['name','year']).mean().reset_index()
        dfTemp =pd.merge(dfTotalCases, dfMean, how='outer', left_on=['year','name'], right_on = ['year','name'])
        fig = go.Figure()
        names=dfTemp.name.unique()
        for name in names:
            dfTempTemp=dfTemp.loc[(dfTemp['name']==str(name))]
            fig.add_trace(go.Scatter(
                x=dfTempTemp['date1'],
                y=dfTempTemp[str(disease)+"_x"],
                mode='lines',
                name='Total Cases per month - '+name
            ))
            fig.add_trace(go.Scatter(
                x=dfTempTemp['date1'],
                y=dfTempTemp[str(disease)+'_y'],
                mode='lines',
                name='Average Cases per year - '+name
            ))
